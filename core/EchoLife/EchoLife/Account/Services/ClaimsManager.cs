...
public static class ClaimsManager
{
    ...
    {
        return principal
            .Claims.FirstOrDefault(c => c.ValueType == ClaimTypes.NameIdentifier)
            ?.Value;
    }

    public static string EnsureGetUserId(ClaimsPrincipal principal)
    {
        return principal.Claims.FirstOrDefault(c => c.ValueType == ClaimTypes.NameIdentifier)?.Value
            ?? throw new ForbiddenException("Missing necessary identifiers.");
    }

    public static string? GetUsername(ClaimsPrincipal principal)
    {
        return principal.Claims.FirstOrDefault(c => c.ValueType == ClaimTypes.Name)?.Value;
    }
}
