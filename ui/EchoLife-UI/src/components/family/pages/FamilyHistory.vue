<template>
    <NewFamilyHistory v-model:status="newLifeHistoryStatus" v-model:list="histories"
        v-model:familyId="route.params.familyId"></NewFamilyHistory>
...
import { GetMyFamilyHistoriesAsync } from '../utils/familyHelper';
import NewFamilyHistory from '../NewFamilyHistory.vue';
...
async function GetMyHistories() {
    ...
    loading.value = true
    pageInfo.value.familyId = route.params.familyId
    var { result, response } = await GetMyFamilyHistoriesAsync(pageInfo.value)
    if (result) {
